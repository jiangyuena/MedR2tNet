...
if current:
    state_dict = torch.load('results/current_checkpoint.pth')
    model_state_dict = state_dict['state_dict']
    model.load_state_dict(model_state_dict) #, strict=False
    model.to(torch.device('cuda'))

    # model.load_state_dict(torch.load('checkpoint/current_checkpoint.pth', map_location=torch.device('cpu')[state_dict]))#[state_dict])
else:
    state_dict = torch.load('results/model_best.pth', map_location=torch.device('cpu'))
    model_state_dict = state_dict['state_dict']
    model.load_state_dict(model_state_dict)
    model.to(torch.device('cuda'))#, strict=False
    # model.load_state_dict(torch.load('checkpoint/model_best.pth', map_location=torch.device('cpu')[state_dict]))  #[state_dict])
